...
for line in fh3:
	...
	if tmp_list[0][:index] not in genes_bl:  #first time we see the gene in the list
		...
		if h > 0:   # to avoid adding the first line when there is nothing computed yet
			...
			final_genes_bl[gene_name] = [Eukaryota, Bacteria, Archaea, Virus, Undefined, Bad_hits]		
			full_class_bl[gene_name] = list(set(tmp_bl))

		genes_bl.append(tmp_list[0][:index])   # We append the gene name into a the list of 'first time'

		Eukaryota = 0   # We reset all the variables to 0
		Bacteria = 0 
		Archaea = 0 
		Virus = 0
		Undefined = 0
	...
	if tmp_list[2] >= options.identity and float(tmp_list[3])*100/int(tmp_list[12]) >= options.coverage:
		...
		if tmp_list[1].find("gene") != -1:

			index_start = tmp_list[1].index("gene")

		else: 

			tmp_list[1] = tmp_list[1].split('.')[3]

			index_start = 0

		if tmp_list[1].find("Eukaryota") != -1:

			index_end = tmp_list[1].index("Eukaryota")
			tmp_bl += [ x for x in tmp_list[1].replace(',', '_')[index_start:index_end].split("_")[1:] if x != '' and x !='""' ]   ## this may change in the future as the blast file may change their format
			Eukaryota += 1

		elif tmp_list[1].find("Bacteria") != -1:

			index_end = tmp_list[1].index("Bacteria")
			tmp_bl += [ x for x in tmp_list[1].replace(',', '_')[index_start:index_end].split("_")[1:] if x != '' and x !='""' ]
			Bacteria += 1

		elif tmp_list[1].find("Archaea") != -1:

			index_end = tmp_list[1].index("Archaea")
			tmp_bl += [ x for x in tmp_list[1].replace(',', '_')[index_start:index_end].split("_")[1:] if x != '' and x !='""' ]
			Archaea += 1

		elif tmp_list[1].find("Virus") != -1:

			index_end = tmp_list[1].index("Virus")
			tmp_bl += [ x for x in tmp_list[1].replace(',', '_')[index_start:index_end].split("_")[1:] if x != '' and x !='""' ]
			Virus +=1
...
final_genes_bl[gene_name] = [Eukaryota, Bacteria, Archaea, Virus, Undefined, Bad_hits] #since no new gene will be found at the end of the file, the last gene wouldn't appear in the results
full_class_bl[gene_name] = list(set(tmp_bl))
...
if options.output is None:
	...
	fh2 = open("contigs_parser_OMRGC_results_v1.txt", 'w')
...
fh2.write("\n>GeneName - Gene Length (bp) - Num Exons - Total Exon Length (bp) - Mean Exon Length (bp) - Num Introns - Total Intron Length (bp) - Mean Intron Length (bp) - Intron Percent in Gene - Eukaryota Hits - Bacteria Hits - Archaea Hits - Virus Hits - Undefined Hits - Bad_hits - Full Blast Classification (until the end)\n")
...
for element in final_genes:
	...
	if element[0] in contigs_info.keys():
		...
		contig_header = "\t{0[0]:.0f}\t{0[1]:.0f}\t{0[2]:.0f}\t{0[3]:.0f}\t{0[4]:.2f}\t{0[5]:.0f}\t{0[6]:.0f}\t{0[7]:.2f}\t{0[8]:.2%}\t{0[9]:.0f}\t{0[10]:.0f}\t{0[11]:.0f}\t{0[12]:.0f}\t{0[13]:.0f}\t{0[14]:.0f}"
		...
		fh2.write("\n# "+element[0]+contig_header.format([0]*15)+"\n\n")
		...
		for i in range(1, len(element)):
			...
			if element[i] in genes_info.keys():
				...
				if element[i] in genes_bl:
					...
					fh2.write(">"+element[i]+"\t{0[0]}\t{0[1]}\t{0[2]}\t{0[3]:.2f}\t{0[4]}\t{0[5]}\t{0[6]:.2f}\t{0[7]:.2%}\t{1[0]}\t{1[1]}\t{1[2]}\t{1[3]}\t{1[4]}\t{1[5]}\t".format(genes_info[element[i]], final_genes_bl[element[i]])+"\t".join(map(str,full_class_bl[element[i]]))+"\n")

				else:

					fh2.write(">"+element[i]+"\t{0[0]}\t{0[1]}\t{0[2]}\t{0[3]:.2f}\t{0[4]}\t{0[5]}\t{0[6]:.2f}\t{0[7]:.2%}\t{1[0]}\t{1[1]}\t{1[2]}\t{1[3]}\t{1[4]}\t{1[5]}\n".format(genes_info[element[i]], [0]*6))
